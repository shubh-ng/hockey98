...
if(!isset($_SESSION['userId'])) {
  header("Location: index?redirectUrl=activate-topup");
  exit();
} 

require 'model/Epin.php';
$epinInfo = new EpinInfo();
$epins = $epinInfo->getTopups($_SESSION['userId'], 0);
if($epins['status']) {
    $epins = $epins['data'];
}else {
    $epins = [];
...
<div class="card-body px-lg-5 pt-0">
    ...
    <!-- Form -->
    <form class="needs-validation" style="color: #757575;" novalidate id="activateTopupForm" >
        ...
        <div class="form-row">
            ...
            <div class="col">
                <!-- E-Pin -->
                <select class="mdb-select md-form" name="topupId">
                    <?php
                        foreach ($epins as $epin) {
                            ?>
                            <option value="<?php echo $epin['epinId']; ?>"><?php echo $epin['epinId']." - ".$epin['cost']; ?></option>
                            <?php
                        }
                        if(empty($epins)) {
                            ?>
                            <option value="" disabled>NO TOP UP AVAILABLE</option>
                            <?php
                        }
                    ?>
                </select>
